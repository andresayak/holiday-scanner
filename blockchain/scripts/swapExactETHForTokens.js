...
async function main() {
    ...
    const amountOutMin = (await router.getAmountOut(amountIn, reservers[0], reservers[1])).mul(5).div(10);
    ...
    await ethers.provider.send("evm_setIntervalMining", [15000]);
    const txSwap = await router.populateTransaction.swapExactETHForTokens(amountOutMin, [
            WETH.address,
            token.address,
        ], owner.address,
        deadline,
